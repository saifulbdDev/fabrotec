...
const Footer = () => {
  return (
    <footer className="bg-white border-t ">
      <div className="mx-auto max-w-7xl px-6 py-2 md:flex md:items-center md:justify-between lg:px-2">
        <div className="flex justify-center space-x-6 md:order-3">
          {navigation.social.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-gray-400 hover:opacity-50  border-primary-400 border-2 px-2.5 rounded-full py-2.5 hover:text-gray-500 dark:text-[#B8B8B8]">
        ...
        <div className="mt-8 md:order-1 md:mt-0">
          <Link href="/" className="-m-1.5 p-1.5">
            ...
            <h2 className="text-2xl">Saiful</h2>
          </Link>
        </div>
      </div>
    </footer>
